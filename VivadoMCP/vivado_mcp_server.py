...
#                         expect, before, pid, isalive, close, terminate)
#   * _build_vivado_session — spawn helper using cmd.exe -> vivado.bat
...
# Match /mnt/<drive>/...  for textual translation to <DRIVE>:/...  This is
# applied to every TCL command before it reaches Vivado, because Vivado is a
# Windows binary that interprets unprefixed paths as local-drive paths.  Only
# affects paths inside TCL strings; non-path content is untouched.
_WSL_PATH_RE = _re.compile(r'/mnt/([a-z])/')


def _translate_wsl_paths_in_tcl(command: str) -> str:
    """Rewrite /mnt/X/... -> X:/... so Windows Vivado can find the file."""
    return _WSL_PATH_RE.sub(lambda m: f"{m.group(1).upper()}:/", command)


class _VivadoSession:
    """Subprocess+PIPE Vivado driver (WSL2-compatible)."""

    def __init__(self, proc: _subprocess.Popen):
        self._proc = proc
        self._lines: _queue.Queue = _queue.Queue()
        self._lock = _threading.Lock()
        self._reader = _threading.Thread(target=self._read_stdout, daemon=True)
        self._reader.start()
    ...
    def send_command(self, command: str, timeout: float = 300.0) -> str:
        # ENV-COMPAT: translate WSL /mnt/X/... paths -> X:/... before they
        # reach Windows Vivado.  Pure textual rewrite; no semantic change for
        # commands that don't reference WSL paths.
        command = _translate_wsl_paths_in_tcl(command)
        with self._lock:
            sentinel = f"VIVADO_DONE_{_uuid.uuid4().hex[:12]}"
            payload = (
                f"set __rc__ [catch {{{command}}} __rv__]\n"
                f"if {{$__rc__ == 0 && [string length $__rv__] > 0}} {{puts $__rv__}}\n"
                f"if {{$__rc__ != 0}} {{puts \"TCL ERROR: $__rv__\"}}\n"
                f"puts {{{sentinel}}}\n"
            )
            ...
            while True:
                ...
                if remaining <= 0:
                    raise TimeoutError(f"Vivado command timed out after {timeout}s")
                try:
                    line = self._lines.get(timeout=min(1.0, remaining))
                except _queue.Empty:
                    continue
                if line is None:
                    raise RuntimeError("Vivado process ended unexpectedly")
                if sentinel in line:
                    break
...
class _PexpectShim:
    """pexpect.spawn-API adapter over _VivadoSession.

    Upstream code uses the sendline+expect+before idiom.  This shim implements
    that surface so no caller needs to change.
    """

    def __init__(self, session: _VivadoSession):
    ...
    def expect(self, pattern, timeout: float = 300.0) -> int:
        if self._pending_cmd is None:
            return 0
        try:
            self.before = self._session.send_command(self._pending_cmd, timeout=timeout)
        except TimeoutError as exc:
            raise pexpect.TIMEOUT(str(exc)) from exc
        except RuntimeError as exc:
            raise pexpect.EOF(str(exc)) from exc
        finally:
    ...
    def close(self, force: bool = False) -> None:
        self._session.terminate(force=force)
...
def _to_windows_path(wsl_path: str) -> str:
    """WSL2 path -> Windows path via wslpath -w; passthrough on failure."""
    try:
...
def _build_vivado_session(log_file=None, journal_file=None) -> _VivadoSession:
    ...
    proc = _subprocess.Popen(
        ...
        env=env,
        start_new_session=_leak_fix,
    )
    if _leak_fix:
        logger.info(f"engine-leak fix: Vivado spawned in its OWN session/pgroup "
...
def cleanup_vivado():
    ...
    _leak_fix = os.environ.get(
        "FPL26_VIVADO_LEAK_FIX", "1").strip().lower() in ("1", "true", "on", "yes")
    # Close stdin first: the engines idle in futex_wait forever precisely because the
    # write end never closed, so they never get EOF. Do this before the kill so a
    # well-behaved engine can exit on its own.
    # Chain is _PexpectShim._session -> _VivadoSession._proc -> Popen.stdin.
    try:
        _sess = getattr(_vivado_process, "_session", None)
        _proc = getattr(_sess, "_proc", None) if _sess is not None else None
        if _proc is not None and getattr(_proc, "stdin", None):
            _proc.stdin.close()
    except Exception:
        pass
    if _vivado_pid:
        ...
        if _leak_fix:
            try:
                ...
                if _pgid != os.getpgrp():
                    ...
                    killed_group = True
                    logger.info(f"engine-leak fix: killpg({_pgid}) — wrapper, loader and "
                                f"engine all terminated together.")
                else:
                    logger.warning("engine-leak fix: vivado shares OUR process group; "
                                   "refusing killpg, falling back to single-PID kill.")
...
def start_vivado(log_file: Optional[str] = None, journal_file: Optional[str] = None) -> pexpect.spawn:
    ...
    # ENV-COMPAT PATCH: replace pexpect.spawn with subprocess+PIPE driver.
    # Upstream pexpect/PTY hangs on WSL2 (Windows ConPTY).  _PexpectShim keeps
    # the rest of upstream code (sendline/expect/before pattern) unchanged.
    logger.info("ENV-COMPAT: spawning Vivado via subprocess PIPE (cmd.exe -> vivado.bat)")
    _session = _build_vivado_session(log_file=log_file, journal_file=journal_file)
    _vivado_process = _PexpectShim(_session)
    _vivado_pid = _session.pid
