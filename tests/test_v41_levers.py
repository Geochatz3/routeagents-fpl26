"""Test opt-in controls for candidate finalization, retiming, and route budgeting.

Finalization skips structural reopening only when the recorded MD5 digest and
file size still match; any mismatch uses full structural validation. Own-front
retiming selects one shallow candidate from the input WNS band and suppresses
competing retiming work so only one candidate consumes runtime. The mid-band
route rung claims its budget before handback decisions. Feature flags default
off, are enabled through both Makefile launch branches, and remain subordinate
to kill switches. Feature selection uses measured characteristics rather than
design names or digests; digests identify candidate files only.
Disabled-feature audit output is gated by `v40_flag_env_present`, and disabled
flags preserve baseline behavior.
"""
...
class TestOwnfrontFrontSelect:
    """Select the retiming front from negative input WNS magnitude.

    Magnitudes in [0.60, 1.00) ns select ETO, while [1.00, 1.05] ns select WLD.
    Values outside these bands, missing values, nonnegative WNS, and malformed
    input return `None`.
    """
    ...
    def test_band_constants(self):
        ...
        assert d.ETO_RETIME_WNS_MAG_MIN_NS == 0.60      # fallback band unchanged
...
class TestOwnfrontFrozenChains:
    ...
    def test_chains_differ_in_front_only(self):
        ...
        differs (the measured rule: retime pays on the OWN winning front)."""
        assert d.WLD_RETIME_TCL[3:] == d.ETO_RETIME_TCL[3:]
        assert d.WLD_RETIME_TCL[:2] == d.ETO_RETIME_TCL[:2]
        assert d.WLD_RETIME_TCL[2] != d.ETO_RETIME_TCL[2]

    def test_retime_after_place_in_both(self):
        for chain in (d.ETO_RETIME_TCL, d.WLD_RETIME_TCL):
            steps = list(chain)
    ...
    def test_candidate_selects_between_the_two_frozen_chains(self):
        ...
        assert ('chain = ETO_RETIME_TCL if front == "eto" '
...
class TestOwnfrontGateArithmetic:
    ...
    def test_vex2_gate_untouched(self):
        ...
        assert (d.RECIPE_PASS_TIMEOUT_FACTOR * d.ETO_RETIME_EXPECTED_S
                + d.ETO_RETIME_OVERHEAD_RESERVE_S + 1100.0) == 2330.0

    def test_ownfront_single_need(self):
        """Verify that one retiming candidate fits the remaining runtime budget.

        The candidate requires 2,420 s from 1.5 × 520 + 540 + 1,100 and must
        fit within the 3,200 s available after the primary pass. Competing
        retiming candidates must defer rather than stack against this budget.
        """
    ...
    def test_expected_covers_both_measured_chain_ranges(self):
        ...
        assert d.OWNFRONT_RETIME_EXPECTED_S >= d.ETO_RETIME_EXPECTED_S

    def test_one_candidate_shape_fits_where_stacking_did_not(self):
        """Ensure candidate scheduling reserves enough runtime for one retiming
        attempt.

        Only one retiming candidate may consume the 2,420 s allowance; stacking
        candidates can exhaust the remaining runtime before the intended
        attempt starts.
        """
        ...
        # Stacked triggers can leave the secondary action below its required budget.
        assert 2195.0 < need
...
class TestOwnfrontSharedFfAuditReuse:
    ...
    def test_candidate_calls_shared_helpers(self):
        src = inspect.getsource(
            d.DCPOptimizer._maybe_run_ownfront_retime_candidate)
        assert "_eto_retime_ff_count" in src
        assert "eto_retime_ff_drift_ok" in src
    ...
    def test_probe_tag_default_preserves_vex2_log_bytes(self):
        sig = inspect.signature(d.DCPOptimizer._eto_retime_ff_count)
        assert sig.parameters["tag"].default == "ETO-RETIME"

    def test_drift_gate_still_fails_closed(self):
        assert d.eto_retime_ff_drift_ok(None, 1682) is False
        assert d.eto_retime_ff_drift_ok(1678, None) is False
    ...
    def test_sentinel_parse_still_fails_closed(self):
        assert d.eto_retime_parse_ffcount("FFCOUNT=1678") == 1678
        assert d.eto_retime_parse_ffcount("1678") is None
...
class TestOwnfrontCandidateSiteDiscipline:
    ...
    def test_vex2_defers_when_ownfront_armed(self):
        """Enforce the one-candidate retiming rule.

        The first candidate defers and logs the reason before band or runtime
        checks when own-front retiming is effectively enabled. Kill-switch
        resolution is part of the enablement check, so disabling or killing
        own-front retiming leaves the baseline candidate path active.
        """
        src = inspect.getsource(
            d.DCPOptimizer._maybe_run_eto_retime_candidate)
    ...
    def test_vex2_own_gates_untouched_after_the_defer(self):
        ...
        src = inspect.getsource(
            d.DCPOptimizer._maybe_run_eto_retime_candidate)
        for key in ("ETO-RETIME: skipped reason=out_of_band ",
                    "ETO-RETIME: skipped reason=wall ",
                    "ETO-RETIME: fired ",
                    "ETO-RETIME: ff-audit ",
                    "ETO-RETIME: result "):
            assert key in src, key
    ...
    def test_ordered_after_vex2_candidate(self):
        """Preserve candidate call order in the shallow bracket.

        The baseline candidate runs first and may defer when own-front retiming
        is enabled; the own-front candidate runs second. Disabled flags retain
        the baseline call shape.
        """
        src = inspect.getsource(d.DCPOptimizer._run_recipe_pass_inner)
        i_vex2 = src.index("_maybe_run_eto_retime_candidate")
        i_own = src.index("_maybe_run_ownfront_retime_candidate")
        assert i_vex2 < i_own

    def test_subband_carveout_precedes_the_pass_body(self):
...
class TestMakefileArming:
    ...
    v4.1 flag — one branch armed alone measures nothing."""
...
# Midband route-rung behavior for designs with moderate negative slack.


class TestBreakRungChain:
    """The break chain is FROZEN verbatim from the recorded dev-machine
    evidence (-0.612/438.79 deterministic x4, in-session and
    cross-machine): unroute -> retime -> poAE -> route AE from the
    BANKED BEST state."""

    def test_frozen_steps_verbatim(self):
        assert d.MIDBAND_BREAK_RUNG_TCL == (
            "route_design -unroute",
            "phys_opt_design -retime",
            "phys_opt_design -directive AggressiveExplore",
            "route_design -directive AggressiveExplore",
    ...
    def test_no_unplace_no_replace(self):
        ...
        for step in d.MIDBAND_BREAK_RUNG_TCL:
    ...
    def test_tail_shared_with_retime_candidates(self):
        ...
        assert d.MIDBAND_BREAK_RUNG_TCL[1:] == d.ETO_RETIME_TCL[3:]
    ...
    def test_v40_explore_constants_retained_as_history(self):
        ...
        assert d.MIDBAND_ROUTE_RUNG_EXPECTED_S == 500.0
        src = inspect.getsource(
            d.DCPOptimizer._maybe_run_midband_route_rung_postloop)
        assert "MIDBAND_ROUTE_RUNG_TCL" not in src
        assert "MIDBAND_ROUTE_RUNG_EXPECTED_S" not in src
        assert "MIDBAND_BREAK_RUNG_TCL" in src
...
class TestBreakRungPricing:
    ...
    def test_expected_covers_measured_cost(self):
        ...
        assert d.MIDBAND_BREAK_RUNG_EXPECTED_S == 810.0
        assert d.MIDBAND_BREAK_RUNG_EXPECTED_S >= 806.0
    ...
    def test_need_arithmetic(self):
        ...
        assert d.MIDBAND_BREAK_RUNG_EXPECTED_S + overhead == 1350.0
        assert (d.RECIPE_PASS_TIMEOUT_FACTOR
                * d.MIDBAND_BREAK_RUNG_EXPECTED_S) == 1215.0
...
class TestBreakRungSiteDiscipline:
    ...
    def test_handback_defer_removed(self):
        """Claim route-rung runtime before evaluating the handback exit.

        Handback state may be observed for logging, but it must not defer the
        rung or prevent it from reserving its budget.
        """
        src = self._src()
    ...
    def test_gate_order_band_then_best_then_wall(self):
        """Verify scope gates precede the priced-wall check.

        Band eligibility runs first, banked-best eligibility second, and the
        wall check last. This ordering bounds handback reallocation to eligible
        designs and makes the wall guard fail closed.
        """
        src = self._src()
    ...
    def test_disabled_audit_gated_behind_env_present(self):
        src = self._src()
        i_gate = src.index('v40_flag_env_present("FPL26_MIDBAND_ROUTE_RUNG"')
...
class TestBreakRungSlotOrdering:
    ...
    def test_rung_before_handback_exit_decision(self):
        ...
        i_rung = src.index("_maybe_run_midband_route_rung_postloop")
...
class TestSpamDetFlagDiscipline:
    def test_default_off(self):
        """No env set -> OFF.  The python default must never arm a lever;
        arming is Makefile-only (never-touch-validated-paths rule)."""
        assert d.shallow_determinizer_enabled() is False

    @pytest.mark.parametrize("val", ["1", "true", "on", "yes", " 1 ", "TRUE"])
    def test_armed_by_truthy(self, val, monkeypatch):
        monkeypatch.setenv("FPL26_SHALLOW_DETERMINIZER_CANDIDATE", val)
        assert d.shallow_determinizer_enabled() is True
    ...
    def test_kill_switch_wins(self, monkeypatch):
        monkeypatch.setenv("FPL26_SHALLOW_DETERMINIZER_CANDIDATE", "1")
        monkeypatch.setenv("FPL26_NO_SHALLOW_DETERMINIZER_CANDIDATE", "1")
        assert d.shallow_determinizer_enabled() is False
...
class TestSpamDetBand:
    """Recognize the lower deterministic slack band.

    Input slack magnitudes in [0.60, 0.90) ns are included: the lower bound is
    ...
    def test_in_band(self, wns):
        assert d.shallow_det_subband_match(wns) is True
    ...
    def test_out_of_band(self, wns):
        assert d.shallow_det_subband_match(wns) is False

    def test_band_constants(self):
        """Verify the retiming bands form an exact partition.

        The intervals [0.60, 0.90) and [0.90, 1.05] have neither a gap nor an
        overlap; their endpoints are pinned by adjacent policy boundaries.
        """
        assert d.SHALLOW_DET_WNS_MAG_MIN_NS == 0.60
        assert d.SHALLOW_DET_WNS_MAG_MIN_NS == d.ETO_RETIME_WNS_MAG_MIN_NS
        assert d.SHALLOW_DET_WNS_MAG_MAX_NS == 0.90
        assert (d.SHALLOW_DET_WNS_MAG_MAX_NS
    ...
    def test_boundary_belongs_to_ownfront(self):
        ...
        assert d.shallow_det_subband_match(-0.90) is False
    ...
    def test_every_boundary_value_covered_by_exactly_one(self):
        for wns in (-0.60, -0.686, -0.899, -0.90, -0.93, -0.946, -1.05):
            n = int(d.shallow_det_subband_match(wns)) + int(
                d.ownfront_retime_front(wns) is not None)
            assert n == 1, wns


class TestSpamDetFrozenChain:
    """The four drilled steps VERBATIM (dev machine, x3 bit-identical
    ...
    def test_normalization_prefix_matches_vex2_chain(self):
        assert d.SHALLOW_DET_TCL[:2] == d.ETO_RETIME_TCL[:2] == (
    ...
    def test_no_unroute_between_the_route_steps(self):
        ...
        undrilled chain."""
        i_explore = d.SHALLOW_DET_TCL.index("route_design -directive Explore")
        i_ae = d.SHALLOW_DET_TCL.index(
            "route_design -directive AggressiveExplore")
        assert i_ae == i_explore + 1
        assert "route_design -unroute" not in d.SHALLOW_DET_TCL[i_explore:]

    def test_s6_ordering_explore_before_aggressive(self):
        """S6 ordering law: Explore first, then AggressiveExplore."""
        assert (d.SHALLOW_DET_TCL.index("route_design -directive Explore")
                < d.SHALLOW_DET_TCL.index(
                    "route_design -directive AggressiveExplore"))

    def test_afwr_is_the_terminal_step(self):
        assert d.SHALLOW_DET_TCL[-1] == (
            "phys_opt_design -directive AlternateFlowWithRetiming")

    def test_no_bare_retime_step(self):
        """Verify the optimization chain contains no bare retiming step.

        Retiming must be exposed only through the AFWR flow, not through a
        direct `phys_opt_design -retime` command.
        """
        assert "phys_opt_design -retime" not in d.SHALLOW_DET_TCL
...
class TestSpamDetGateArithmetic:
    ...
    def test_shallow_det_need(self):
        """need3 = 1.5x390 (cap 585) + 540 overheads + 1100 LLM floor
        = 2225 s."""
        cap = d.RECIPE_PASS_TIMEOUT_FACTOR * d.SHALLOW_DET_EXPECTED_S
        assert cap == 585.0
        need = cap + d.SHALLOW_DET_OVERHEAD_RESERVE_S + 1100.0
        assert need == 2225.0
        assert d.SHALLOW_DET_OVERHEAD_RESERVE_S == 540.0
        assert d.SHALLOW_DET_OVERHEAD_RESERVE_S == (
            d.RECIPE_PASS_MEASURE_RESERVE_S + d.RECIPE_PASS_STORE_RESERVE_S
            + d.RECIPE_PASS_REGISTER_RESERVE_S)

    def test_expected_is_the_measured_chain_cost(self):
        """~390 s x3 bit-identical on the dev machine — deterministic chain, so
        expected pins to the measured cost."""
        assert d.SHALLOW_DET_EXPECTED_S == 390.0

    def test_swap_makes_the_fire_affordable(self):
        """Verify candidate deferral leaves enough wall budget for the
        determinizer.

        Within [0.60, 0.90), only the primary path may spend before this gate.
        Even its slow allocation must leave at least the budget required to run
        the determinizer.
        """
        max_remaining = 3200.0
        need = (d.RECIPE_PASS_TIMEOUT_FACTOR * d.SHALLOW_DET_EXPECTED_S
                + d.SHALLOW_DET_OVERHEAD_RESERVE_S + 1100.0)
        assert need == 2225.0
        ...
        vex2_cap = d.RECIPE_PASS_TIMEOUT_FACTOR * d.ETO_RETIME_EXPECTED_S
...
class TestSpamDetFfAudit:
    ...
    def _src(self):
        return inspect.getsource(
            d.DCPOptimizer._maybe_run_shallow_determinizer_candidate)

    def test_shared_helpers_no_fork(self):
        src = self._src()
        assert "_eto_retime_ff_count" in src
        assert "eto_retime_ff_drift_ok" in src
        assert 'tag="SHALLOW-DET"' in src
        assert not hasattr(d, "shallow_det_parse_ffcount")
        assert not hasattr(d.DCPOptimizer, "_shallow_det_ff_count")
    ...
    def test_probes_bracket_the_afwr_step(self):
        ...
        assert d.SHALLOW_DET_TCL.count(
    ...
    def test_audit_fails_closed(self):
        src = self._src()
        assert "eto_retime_ff_drift_ok(ff_before, ff_after)" in src
...
class TestSpamDetSiteDiscipline:
    ...
    def test_disabled_audit_line_gated_behind_env_present(self):
        src = self._src()
        i_gate = src.index(
            'v40_flag_env_present("FPL26_SHALLOW_DETERMINIZER_CANDIDATE"')
        i_line = src.index("SHALLOW-DET: skipped reason=disabled")
        assert i_gate < i_line

    def test_stable_log_keys(self):
        src = self._src()
        for key in ("SHALLOW-DET: skipped reason=disabled ",
                    "SHALLOW-DET: skipped reason=out_of_band ",
                    "SHALLOW-DET: skipped reason=wall ",
                    "SHALLOW-DET: fired ",
                    "SHALLOW-DET: ff-audit ",
                    "SHALLOW-DET: result "):
            assert key in src, key

    def test_gate_order_flag_band_wall_fire(self):
        src = self._src()
        i_flag = src.index("if not shallow_determinizer_enabled():")
        i_band = src.index("shallow_det_subband_match")
        i_wall = src.index("_budget_remaining")
        i_fired = src.index("SHALLOW-DET: fired wns_in=")
        assert i_flag < i_band < i_wall < i_fired

    def test_ordered_after_vex2_and_ownfront(self):
        """Verify shallow-bracket candidates execute in the required order.

        The validated-spend candidate runs first, the front candidate second,
        and the determinizer last. Disabling the determinizer must preserve the
        preceding call sequence.
        """
        src = inspect.getsource(d.DCPOptimizer._run_recipe_pass_inner)
        i_vex2 = src.index("_maybe_run_eto_retime_candidate")
        i_own = src.index("_maybe_run_ownfront_retime_candidate")
        i_det = src.index("_maybe_run_shallow_determinizer_candidate")
        assert i_vex2 < i_own < i_det

    def test_vex2_defers_to_determinizer_in_its_band(self):
        """Verify the competing candidate defers to the determinizer only in its
        assigned band.

        Deferral requires both the feature flag and membership in [0.60, 0.90).
        The deferral decision must precede the competing candidate's own band
        check.
        """
        src = inspect.getsource(
            d.DCPOptimizer._maybe_run_eto_retime_candidate)
        i_defer = src.index("reason=shallow_det_supersedes")
        i_cond = src.index("shallow_determinizer_enabled() and shallow_det_subband_match(wns_in)")
        i_band = src.index("eto_retime_subband_match")
    ...
    def test_swap_band_alignment(self):
        ...
        assert d.shallow_det_subband_match(-0.686) is True
        assert d.shallow_det_subband_match(-0.899) is True
        assert d.shallow_det_subband_match(-0.90) is False
        assert d.shallow_det_subband_match(-0.946) is False

    def test_pristine_reopen_before_the_chain(self):
        src = self._src()
        i_reopen = src.index("_presweep_reopen")
        i_loop = src.index("for i, step in enumerate(SHALLOW_DET_TCL, 1):")
        assert i_reopen < i_loop

    def test_wall_skip_is_fail_closed(self):
        src = self._src()
        assert "fail closed" in src
        i_wall = src.index("SHALLOW-DET: skipped reason=wall ")
        i_fired = src.index("SHALLOW-DET: fired wns_in=")
        assert i_wall < i_fired


class TestSpamDetManifestAndMakefile:
    """Provenance + arming surface (the 'Makefile not in the ship
    ...
    def test_both_branches_armed(self, branches):
        expected = ("FPL26_SHALLOW_DETERMINIZER_CANDIDATE="
                    "$(if $(SHALLOW_DET),$(SHALLOW_DET),1)")
        for branch in branches:
            assert expected in branch
